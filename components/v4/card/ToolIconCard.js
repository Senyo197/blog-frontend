...
const ToolIconCard = ({ tool, withBackground, small, logo }) => {
  ...
  return (
    <div>
      ...
      >
        <div
          className={`${withBackground ? "hover:shadow-md hover:scale-[1.005] transition-all duration-400 bg-white p-3 shadow-sm border border-gray-200/70 border-1 rounded-2xl" : ""} w-full h-auto cursor-pointer flex flex-col group`}
        >
          <div className="flex flex-row rounded-xl justify-between gap-3">
            <div className="flex justify-start w-full">
              <div
                className={`${small ? "h-12 w-12" : "w-[56px] h-[56px]"} border border-gray-300/30 mr-2 relative rounded-xl overflow-hidden group-hover:scale-[1.03] group-hover:shadow-sm flex-none transition-all duration-700`}
              >
                <Image
                  tabIndex={0}
                  lazy={true}
            ...
            <div className="flex flex-col z-10 justify-center">
              <span className="text-xs uppercase bg-gray-100 font-medium py-0.5 px-3 text-blue-600 border border-black border-opacity-5 rounded-full">
                Get
              </span>
            </div>
          </div>
